...
#define PROID		"493504"    //产品ID

#define AUTH_INFO	"123456"	//鉴权信息

#define DEVID		"904802922"	//设备ID
...
extern float value_mq135;		//一氧化碳浓度
extern u8 status_sr501;		    //人体红外检测到的值
extern int max_mq135;			//一氧化碳浓度阈值
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	//烟雾
	memset(text, 0, sizeof(text));
	sprintf(text, "MQ135,%.1fppm;", value_mq135);
	strcat(buf, text);
	//湿度
	memset(text, 0, sizeof(text));
...
//	//空气质量阈值
//	memset(text, 0, sizeof(text));
//	sprintf(text, "MAX_MQ135,%d;", max_mq135);
//	strcat(buf, text);
...
//				printf("Tips:	MQTT Publish Send OK\r\n");
	...
	if(dataPtr != NULL && result != -1)					//如果找到了
	{
		...
		num = atoi((const char *)numBuf);				//转为数值形式
		 ...
		 if(strstr((char *)req_payload, "MAX_MQ135"))
		{
			max_mq135 = num;
		}
